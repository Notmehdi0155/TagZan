...
def save_collection(code, file_ids, cover_id=None, caption=None):
    ...
    c.execute("REPLACE INTO collections (code, file_ids, cover_id, caption) VALUES (?, ?, ?, ?)",
              (code, json.dumps(file_ids), cover_id, caption))
    conn.commit()
    conn.close()
...
def get_collection(code):
    conn = sqlite3.connect("data.db")
    c = conn.cursor()
    c.execute("SELECT file_ids, cover_id, caption FROM collections WHERE code=?", (code,))
    row = c.fetchone()
    conn.close()
    if row:
        file_ids = json.loads(row[0])
        return file_ids, row[1], row[2]
    return None